...
# 3. 🎯 [최종 정밀 실측 좌표 교정] 각 알파벳 슬롯 사각형의 정확한 수평/수직 한가운데 좌표값
# 위로 밀리던 오차를 해결하기 위해 세로축(top) 퍼센트를 정밀하게 아래로 내려 보정했습니다.
SEAT_COORDINATES = {
    # 1번째 줄 (A ~ F) -> 기존 14.5%에서 19.5%로 아래로 하향 조정
    ...
    # 4번째 줄 (R ~ W) -> 기존 88.7%에서 93.8%로 아래로 하향 조정
